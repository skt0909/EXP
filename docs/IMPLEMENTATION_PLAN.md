...
**Phase 4e. The current gameweek, and locked/scored state on the dashboard. — DONE.** Three fixes, one theme: `GET /gameweeks/current` was answering the wrong question, in the wrong season.
  - **A. The season leak.** `Game_logic/fixtures.py::CURRENT_GAMEWEEK_QUERY` ranked deadlines across **every** season in `ml.fixtures` with no filter, and the simulation seasons carry real timestamps — so a `SIM38OK` fixture could win and every page in the app would show a simulation gameweek. Proven first by `test_a_simulation_season_never_wins_the_current_gameweek`, which asserted `'SIM38OK' != 'SIM38OK'` before the fix. Filter added: `season ~ '^[0-9]{4}-[0-9]{2}$'`.
    - **The six other unfiltered production queries are now fixed** (see A-ter): `GameEngine/gameweek_finalize.py`, `GameEngine/gameweek_lock.py` ×2, `Data/live_poll.py`, `Predict/prediction_scheduling.py`. The prediction one **corroborates** a long-standing UNEXPECTED failure — `test_schedule_predictions_no_op_when_nothing_needs_predicting` — the same root cause, confirmed structurally rather than guessed. Six further Dream11 queries remain out of scope.
  ...
  - **A-ter. The leak closed everywhere, with one definition.** New module **`Shared/seasons.py`** owns `REAL_SEASON_PATTERN` (`^20[0-9]{2}-[0-9]{2}$`), `REAL_SEASON_RE`, `real_season_sql(column)` and `is_real_season(season)`. The pattern had been written out twice, tightened in one of them, and the two disagreed — the same "one source of truth" argument `DEADLINE_OFFSET_MINUTES` already won. Applied to all seven sites: `Game_logic/fixtures.py`, `Results/scoring_job.py` (`REAL_SEASON_RE` now imports rather than redefines), `GameEngine/gameweek_finalize.py`, `GameEngine/gameweek_lock.py` ×2, `Data/live_poll.py`, `Predict/prediction_scheduling.py`.
    - **These leaked in practice, not hypothetically.** The unfiltered active-gameweek sweep, run against a clone of `fpl_game` on 2026-09-22:

      | window | gameweeks returned | non-real |
      |---|---|---|
      | 5 days | 1 | 0 |
      | 12 days | 79 | **77** (`SIM38OK`, `SIM38TST`, `SIMSMOKE`) |

      The SIM seasons' last kickoffs are 2026-09-11, so they sat inside the live 5-day window **until 2026-09-16** — six days before this audit. Anyone running `refresh_active_gameweeks` in that window handed 77 simulation gameweeks to the scoring job. Today's clean result is a matter of timing, not of the code being right.
    - **`TEST_SEASON` moved `9999-00` → `2099-00`**, in `Tests/conftest.py` (its only definition; 31 test files import it, one literal in `test_auth.py` updated). This is a **coupled** change, not scope creep: `test_scoring_job_tactical.py:136` asserts `re.fullmatch(REAL_SEASON_RE, TEST_SEASON)`, so the codebase already declares that `TEST_SEASON` must be real-shaped — tightening the pattern without moving it would violate an invariant the tests state outright. The shape is preserved exactly: a season is `YYYY-YY` where the second half is the following year's last two digits, and `str(2100)[-2:] == '00'`, so `2099-00` derives as `9999-00` did and remains a season that cannot occur.
    - **A pattern is not a guarantee.** `20YY-YY` cannot distinguish a real season from a test one shaped like it — `2099-00` is deliberately indistinguishable, because the suite must exercise the real-season path. The actual defence is that test and simulation seasons never reach the production database, and the `9998-00` and `9999-00` rows **already in `fpl_game`** are proof that this has failed once. The filter is the second line.
    - **Not re-run.** Testing is paused by instruction, so the suite has not been executed against any of this. Verification was done by importing every changed module and executing each of the seven queries against the `fpl_game` clone; all returned real-season rows only.
...
- ~~The six unfiltered season queries~~ — **closed** in Phase 4e A-ter via `Shared/seasons.py`. What remains is the **six Dream11 queries**, still out of scope, and the fact that `fpl_game` holds `9998-00` and `9999-00` rows at all: the filter hides them, it does not remove them.
