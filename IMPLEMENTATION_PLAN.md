...
**Phase 4b. Integration, the rest.** Wire the dashboard onto the new engine, remove chips/hits/`revert_free_hits`/captaincy, run the full suite including Dream11 and ML tests. **GATE: full suite green.**
  - `DASHBOARD_DATA.md` is the survey: per-player points are **not stored**, and `team_dashboard.py` imports `resolve_autosubs` straight from the classic `Results/scoring.py`, so that module cannot be deleted until the dashboard moves.
  - Ambiguities E1 to E7 in `PHASE4A_REPORT.md`, notably: `Tools/fpl_sim.py` still uses the classic scorer because the new season filter refuses `SIM*`; two scorers now coexist; a batch write is one transaction where the old job committed per manager.
